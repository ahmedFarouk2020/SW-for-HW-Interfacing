...
void EXTI_init(unsigned char portId, unsigned char pinId, unsigned char EXTI)
{
	...
	// enable EXTI on pin
	EXTI_EnableLine(pinId);

	// config trigger edge
	EXTI_trigger(FALLING_EDGE, pinId);

	// enable NVIC EXTI
	EXTI_enableNVIC(EXTI);
}
...
{
	 function_callback0 = ptr_function;
}

void EXTI_setCallBack1(void(*ptr_function)(void))
{
	 function_callback1 = ptr_function;
}
